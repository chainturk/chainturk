...
# Chainturk Multi-installer
# a one line clone-and-compile for chainturk:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/chainturk/chainturk/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/chainturk ]; then
            read -r -p "${1:-chainturk directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old chainturk directory" && echo
                    rm -rf "$PWD"/chainturk
                    ;;
                *)
                    _fail "chainturk directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir chainturk
        git clone -b master -q https://github.com/chainturk/chainturk chainturk   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd chainturk
    fi
}

_build_chainturk() {
    _note "Building chainturk from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/chainturk')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/chainturk')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Chainturk Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'chainturk/build/src/' for the executible binaries. See 'https://github.com/chainturk/chainturk' for more project support. Cowabunga!"
