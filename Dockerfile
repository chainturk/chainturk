...
# docker run -v $(pwd)/chainturkd:/var/lib/chainturkd -v $(pwd)/wallet:/home/chainturk --rm -ti chainturk:0.2.2
...
ARG CHAINTURK_BRANCH=master
ENV CHAINTURK_BRANCH=${CHAINTURK_BRANCH}
...
RUN apt-get update && \
    ...
    git clone https://github.com/chainturk/chainturk.git /src/chainturk && \
    cd /src/chainturk && \
    git checkout $CHAINTURK_BRANCH && \
    mkdir build && \
    cd build && \
